  ...
  if(rerun == 0){ DNAS = randu<mat>(1100,netDataSize);}
     ...
     for(int gen = 0; gen < generations; gen++){
      winning=  getBest(DNAS, net, X);
	     if(finalizeit == 0){ DNAS = ga_eval(DNAS, net, X ,0.05, 0.01, 0.005, 10.0,0.0005,100.0, finalizeit,winning);}
	     else{DNAS = ga_eval(DNAS, net, X ,0.99, 0.001, 0.00001, 1.0,0.000000001,10, finalizeit,winning);}
